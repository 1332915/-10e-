...
download_file "run.sh" || true
download_file "RUN_ON_PHONE.md" || true
...
echo "   uname -r"
echo ""
echo "3. 运行 exploit (v2, 分阶段!):"
echo ""
echo "   ※ 详细步骤见 RUN_ON_PHONE.md (已下载到本目录)"
echo ""
echo "   先测试漏洞触发 (设备崩溃重启=漏洞确认):"
echo "     sh run_boot.sh -t"
echo ""
echo "   再跑完整模式 (安全: 只写用户内存, 泄露验证):"
echo "     sh run_boot.sh"
echo "     看日志 'CHAIN WALK OK' = 利用链打通"
echo ""
echo "   链打通后试验 SELinux 写 0 (可能崩溃):"
echo "     sh run_boot.sh -s"
echo ""
echo "4. 若 Termux 被 seccomp 拦截, 用 MT 管理器:"
echo "   把 exploit + run.sh 复制到 /sdcard/Download/cve_2026_43499/"
echo "   点击 run.sh → 终端 执行"
echo ""
echo "=== 注意事项 ==="
echo "- 必须先跑 -t 测试模式, 确认 UAF 触发后再跑完整模式"
echo "- 如果设备崩溃重启, 说明漏洞存在但利用参数可能需要调整"
echo "- v2 默认模式不写内核地址 (用户态伪锁泄露验证), 安全"
echo "- 内核若已合并 3bfdc63936dd 补丁则无法触发"
echo "- 检查 /proc/sys/kernel/kptr_restrict, 影响 -s 自动取符号"
echo ""
echo "现在可以运行:"
